...
const StudentManagementList: React.FC = () => {
  ...
  const loadAllBatches = async () => {
    ...
    const list: Batch[] = data.sort((a: Batch, b: Batch) =>
  ...
  const loadStudentsAll = async () => {
    ...
    setStudents(
      Array.isArray(res.data) ? (res.data as unknown as Student[]) : []
    );
  };
  ...
  const loadStudentsNoBatch = async () => {
    ...
    const data = Array.isArray(res.data)
      ? (res.data as unknown as Student[])
      : [];
    const list: Student[] = data.filter((s) => !s.batch);
    setStudents(list);
  };
  ...
  const loadStudentsForBatch = async (batchId: number) => {
    ...
    const data = Array.isArray(res.data) ? res.data : [];
    setStudents(mapBatchStudentsToStudents(data as BatchStudentRow[]));
  };
      ...
      s.id !== userId
        ...
        : {
            ...
            batch: {
              ...
              batchName:
                batches.find((b) => b.id === Number(value))?.name ?? "",
              batchCode:
                batches.find((b) => b.id === Number(value))?.code ?? "",
            },
          }
    );
    setStudents(next);
