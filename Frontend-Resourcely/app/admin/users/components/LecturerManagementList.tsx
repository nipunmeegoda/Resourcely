...
type Department = DepartmentType & { id: number };
...
const LecturerManagementList: React.FC = () => {
  ...
  const [departmentFilter, setDepartmentFilter] =
    useState<DepartmentFilterValue>("ALL");
  ...
  const loadAllDepartments = async () => {
    ...
    const list: Department[] = data
      .filter((d): d is Department => d.id !== undefined)
  ...
  const loadLecturersAll = async () => {
    ...
    setLecturers(
      Array.isArray(res.data) ? (res.data as unknown as Lecturer[]) : []
    );
  };
  ...
  const loadLecturersNoDepartment = async () => {
    ...
    const data = Array.isArray(res.data)
      ? (res.data as unknown as Lecturer[])
      : [];
    const list: Lecturer[] = data.filter((l) => !l.department);
    setLecturers(list);
  };
  ...
  const loadLecturersForDepartment = async (departmentId: number) => {
    ...
    const res = await usersApi.getAllRoleLecturer();
    const allLecturers: Lecturer[] = Array.isArray(res.data)
      ? (res.data as unknown as Lecturer[])
      : [];
    const list = allLecturers.filter(
      (l) => l.department?.departmentId === departmentId
    );
    setLecturers(list);
  };
      ...
      l.id !== userId
        ...
        : {
            ...
            department: {
              ...
              departmentName:
                departments.find((d) => d.id === Number(value))?.name ?? "",
            },
          }
    );
    setLecturers(next);
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(val: DepartmentFilterValue) =>
        ...
        <tbody>
          {lecturers.map((l) => {
            ...
            const currentVal: DepartmentFilterValue =
              l.department?.departmentId != null
                ? `${l.department.departmentId}`
                : "NONE";
